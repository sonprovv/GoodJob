...
class ChatBotRemote(private val chatApiService: ChatApiService) : ChatBotDataSource {
    override suspend fun chatBot(request: String, refrence : ReferenceData): NetworkResult<ChatBotResponse> {
        return safeApiCall {
            chatApiService.chatBot(ChatBotRequest(request, refrence))
        }
    }
    companion object {
        private var instance: ChatBotRemote? = null
        fun getInstance(): ChatBotRemote {
            if (instance == null) instance = ChatBotRemote(RetrofitClient.chatApiService)
            return instance!!
        }
    }
}