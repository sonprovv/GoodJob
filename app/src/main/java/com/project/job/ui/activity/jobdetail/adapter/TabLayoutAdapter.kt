...
class TabLayoutAdapter(activity: FragmentActivity, private val dataJob: DataJobs) : FragmentStateAdapter(activity) {
    override fun getItemCount(): Int = 2

    override fun createFragment(position: Int): Fragment {
        return when (position) {
            0 -> JobInfoFragment.newInstance(dataJob)
            1 -> WorkerOrderFragment.newInstance(dataJob.uid)
            else -> JobInfoFragment.newInstance(dataJob)
        }
    }
}