...
class HealthcareServiceDetailFragment(private val healthcareService: List<HealthcareService?>) : BottomSheetDialogFragment() {
    ...
    private val people = mutableListOf<HealthcareService>()
    ...
    override fun onCreateView(
       ...
       _binding = FragmentHealthcareServiceDetailBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setupButton()
        updatePeopleFromHealthcareServices()
        setupTabLayout()
    }
    private fun updatePeopleFromHealthcareServices() {
        people.clear()
        healthcareService.forEach { service ->
            if (service != null) {
                people.add(
                    HealthcareService(
                        uid = service.uid,
    ...
    private fun setupTabLayout() {
        ...
        binding.tabLayout.apply {
            ...
            tabMode = if (people.size > 2) TabLayout.MODE_SCROLLABLE else TabLayout.MODE_FIXED
        ...
        // Set up ViewPager2
        val adapter = TabLayoutHealthcareAdapter(requireActivity(), rooms = people)
        ...
        tabLayoutMediator = TabLayoutMediator(binding.tabLayout, binding.viewPager2) { tab, position ->
            ...
            tab.customView = createTabView(people[position].serviceName)
        }
