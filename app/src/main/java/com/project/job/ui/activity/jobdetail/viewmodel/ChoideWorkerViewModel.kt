...
class ChoideWorkerViewModel: ViewModel() {
    ...
    fun choiceWorker(token: String, uid: String, status: String) {
        viewModelScope.launch {
            ...
            _error.value = null
            try {
                val response = serviceRepository.updateChoiceWorker(token=token, uid=uid, status=status)
    ...
    fun postReviewWorker(token: String, userID: String, rating: Int, comment: String, workerID: String, orderID: String, serviceType: String) {
        viewModelScope.launch {
            ...
            _error.value = null
            try {
                val response = serviceRepository.postReviewWorker(token=token, userID=userID, rating=rating, workerID = workerID, orderID = orderID, comment=comment, serviceType = serviceType)
